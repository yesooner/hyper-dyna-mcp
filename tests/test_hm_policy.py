...
def test_trusted_generators_not_empty():
    ...
    assert EXECUTION_BLOCKED_GENERATORS["generate_surface_automesh_tcl"] == "surface_automesh"
    assert EXECUTION_BLOCKED_GENERATORS["generate_plain_tetra_tcl"] == "tetmesh_geometry_solid"


def test_forbidden_commands_list():
    assert "*meshdragelements" in FORBIDDEN_MESHING_COMMANDS
    assert "*automesh" in FORBIDDEN_MESHING_COMMANDS
    assert "*meshsolidsonly" in FORBIDDEN_MESHING_COMMANDS
    assert "*tetmesh" in FORBIDDEN_MESHING_COMMANDS
    assert FORBIDDEN_COMMAND_ROUTES["*automesh"] == "surface_automesh"
    assert FORBIDDEN_COMMAND_ROUTES["*tetmesh"] == "tetmesh_geometry_solid"
...
def test_generated_unverified_tetmesh_script_is_blocked():
...
def test_generated_unverified_surface_automesh_script_is_blocked():
    body = '*createmark surfaces 1 "all"\n*automesh 0 2 2'
    script = wrap_generated_tcl("generate_surface_automesh_tcl", body)
    result = check_meshing_rules(script)
    assert result is not None
    assert result["error_type"] == "mesh_route_not_verified"
