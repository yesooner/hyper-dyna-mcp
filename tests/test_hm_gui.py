...
def test_generate_listener_tcl_is_hm2024_compat():
    ...
    assert "proc ::mcp_hm_channels" in script
    assert "file channels" in script
    assert "proc ::mcp_hm_errorinfo" in script
    assert "dict exists" not in script
    assert "\ntry " not in script
    assert "-errorinfo" in script
    assert "fconfigure $chan -blocking 1 -buffering none -translation binary -encoding utf-8" not in script
...
def test_execute_tcl_gui_preserves_raw_mode_for_socket(monkeypatch):
    ...
    def fake_send(script, host="127.0.0.1", port=47883, timeout=120, mode="safe"):
...
def test_plugin_loop_passes_mode_to_socket(monkeypatch):
    ...
    def fake_send(script, timeout=120, mode="safe"):
        captured["script"] = script
    ...
    assert result["success"] is True
    assert captured == {"script": "customhmcommand debug", "timeout": 7, "mode": "raw"}
...
def test_mesh_box_wraps_tetmesh_as_trusted_script(monkeypatch):
    captured = {}
    ...
    assert "MCP_GENERATED_BY=generate_plain_tetra_tcl" in captured["script"]
    assert "# MCP_SCRIPT_BEGIN" in captured["script"]
    assert "*tetmesh 1 1 2.5" in captured["script"]
    assert "# MCP_SCRIPT_END" in captured["script"]
...
def test_create_solid_box_uses_verified_solidblock_route(monkeypatch):
    captured = {}
    ...
    def fake_execute_tcl_gui(script, timeout=30, **kwargs):
        ...
        return {
            ...
            "response": (
                ...
                "SOLIDS_BEFORE=0\n"
                "SOLIDS_AFTER=1\n"
                "SOLIDS_BEFORE_METHOD=entitylist\n"
                "SOLIDS_AFTER_METHOD=entitylist\n"
                "SOLID_ID=1\n"
    ...
    assert result["solid_id"] == 1
    assert result["solids_before"] == 0
    assert result["solids_after"] == 1
    assert result["solids_before_count_method"] == "entitylist"
    assert result["solids_after_count_method"] == "entitylist"
    assert result["solid_count_methods"] == {"before": "entitylist", "after": "entitylist"}
    assert result["created_count"] == 1
    assert result["visual_counts"]["solids"] == 1
    assert result["visual_displayed_counts"]["solids"] == 1
    assert result["visibility"]["solids"] == "displayed"
    assert result["visibility"]["elements"] == "absent"
    assert captured["timeout"] == 30
    assert captured["kwargs"]["enforce_rules"] is False
    assert '*currentcollector comps "solid_component"' in captured["script"]
    assert "*solidblock 0 0 0 1 0 0 0 1 0 0 0 1" in captured["script"]
    assert "catch {set _hdm_solid_id [hm_latestentityid solids]}" in captured["script"]
    assert "catch {set _hdm_solids_before [hm_marklength solids 1]}" in captured["script"]
    assert "catch {set _hdm_solids_after [hm_marklength solids 1]}" in captured["script"]
    assert "set _hdm_solids_before_method entitylist" in captured["script"]
    assert "set _hdm_solids_before_method marklength" in captured["script"]
    assert 'puts "SOLIDS_BEFORE_METHOD=$_hdm_solids_before_method"' in captured["script"]
    assert 'puts "SOLIDS_AFTER_METHOD=$_hdm_solids_after_method"' in captured["script"]
    assert "HDM_VISUAL_REFRESH_BEGIN" in captured["script"]
...
def test_create_solid_box_fails_without_solids_count_increase(monkeypatch):
    ...
    assert "solids_count" in result["error"]
...
def test_gui_modeling_smoke_runs_fe_solid_then_refresh(monkeypatch):
    ...
    assert evidence["criteria"]["visual_display_has_elements_and_solids"] is True
...
def test_gui_modeling_smoke_rejects_refresh_without_fe_and_solid_counts(monkeypatch):
    def fake_create_fe_cube(*args, **kwargs):
        return {"success": True, "element_count": 1}
    ...
    assert result["success"] is False
    assert result["stage"] == "visual_refresh"
    assert result["visual_counts_ok"] is False
    assert result["visual_counts_reason"] == "solids_count_absent"
    assert result["visual_display_ok"] is False
    assert result["runtime_validation_evidence"]["runtime_validated"] is False
    assert result["runtime_validation_evidence"]["criteria"]["visual_counts_have_elements_and_solids"] is False
    assert "FE elements and geometry solids" in result["error"]


def test_gui_modeling_smoke_rejects_hidden_fe_or_solid_display(monkeypatch):
    def fake_create_fe_cube(*args, **kwargs):
        return {"success": True, "element_count": 1}
    ...
    assert result["visual_counts_ok"] is True
    assert result["visual_display_ok"] is False
    assert result["visual_display_reason"] == "solids_not_displayed"
    assert "display state is hidden or unknown" in result["error"]
