# LS-DYNA R13 *CONTACT_AUTOMATIC_SINGLE_SURFACE
# Category: contact
# Entity type: groups
# Card image: CONTACT_AUTOMATIC_SINGLE_SURFACE
# Parameters: SSID(int), MSID(int), SSTYP(int), MSTYP(int), SBOXID(int), MBOXID(int), SPR(int), MPR(int)

*createentity groups name="CONTACT_AUTOMATIC_SINGLE_SURFACE_{{SSID}}" cardimage=CONTACT_AUTOMATIC_SINGLE_SURFACE
*createmark groups 1 "by name" "CONTACT_AUTOMATIC_SINGLE_SURFACE_{{SSID}}"
set _grp_ids [hm_getmark groups 1]
set _grp_id [lindex $_grp_ids end]
if {$_grp_id eq ""} { error "Failed to create CONTACT_AUTOMATIC_SINGLE_SURFACE" }
...
puts "HM_KEYWORD_CREATED keyword=CONTACT_AUTOMATIC_SINGLE_SURFACE entity=groups id=$_grp_id"
