# LS-DYNA R13 *LOAD_PRESCRIBED_MOTION_SET
# Category: load
# Entity type: loadcols
# Card image: LOAD_PRESCRIBED_MOTION_SET
# Parameters: NSID(int), DOF(int), VAD(int), LCID(int), SF(float), VID(int), DEATH(float), BIRTH(float)

*createentity loadcols name="LOAD_PRESCRIBED_MOTION_SET_{{NSID}}" cardimage=LOAD_PRESCRIBED_MOTION_SET
*createmark loadcols 1 "by name" "LOAD_PRESCRIBED_MOTION_SET_{{NSID}}"
set _lc_ids [hm_getmark loadcols 1]
set _lc_id [lindex $_lc_ids end]
if {$_lc_id eq ""} { error "Failed to create LOAD_PRESCRIBED_MOTION_SET" }
...
puts "HM_KEYWORD_CREATED keyword=LOAD_PRESCRIBED_MOTION_SET entity=loadcols id=$_lc_id"
