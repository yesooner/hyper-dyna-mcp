...
```json
{
  "listener_tcl": "F:\\hyper-dyna-mcp\\runs\\hm_gui_listener.tcl",
  "host": "127.0.0.1",
  "port": 47883,
  "listener_version": "2024-compat-v3",
  "source_command": "source \"C:/path/to/hyper-dyna-mcp/runs/hm_gui_listener.tcl\"",
  "start_or_source_command": "if {[llength [info commands mcp_start_on_port]]} {mcp_start_on_port 47883} else {source \"C:/path/to/hyper-dyna-mcp/runs/hm_gui_listener.tcl\"}",
  "next_hypermesh_commands": [
    "catch {mcp_stop}",
    "if {[llength [info commands mcp_start_on_port]]} {mcp_start_on_port 47883} else {source \"C:/path/to/hyper-dyna-mcp/runs/hm_gui_listener.tcl\"}"
  ]
}
```
...
Executes a constrained HyperMesh Python API script through the GUI-side bridge. This remains a HyperMesh-only workflow.
...
Geometry solid creation is a separate route from FE mesh creation. The tool is
implemented through the verified `create_geometry_solid_box` route in
`templates/hm_command_map.json`. The Tcl route uses local Altair script evidence
for `*solidblock`, then validates the run by checking that `solids_count`
increased. Solid counting first tries `hm_entitylist solids id` and falls back
to `*createmark solids 1 all` plus `hm_marklength solids 1` for older
HyperMesh Tcl behavior.
...
If the target HyperMesh 2024/older session rejects `*solidblock`, the tool
returns `success=false` and includes the raw listener response.
...
the FE/solid entities exist and are displayed.
...
Runs a connected-GUI modeling smoke workflow: create one small FE cube, create
one geometry solid box, then request a visualization refresh. The workflow stops
after the first failed stage to avoid repeated listener timeouts.
...
`visual_counts.elements > 0` and `visual_counts.solids > 0`, and exposes
`visual_counts_ok` plus `visual_display_ok` for the count/display gates. The
repo-local Claude smoke command enforces the same gate and prefers those
tool-reported gate fields when present. When display diagnostics are available,
FE elements and solids must be displayed; otherwise the failure reason is
`visual_display_hidden`.
Its JSON output includes `modeling_smoke_failure.reason` when that gate fails.
When it succeeds, `runtime_validation_evidence` records the criteria needed to
promote `create_geometry_solid_box` from `source_verified_runtime_pending` to
runtime validated, including `solids_before`, `solids_after`, `created_count`,
`solid_count_methods`, visual counts, displayed counts, count methods, display
count methods, and visibility. It also includes
`command_map_update_candidate`, a non-automatic review hint that points to
`templates/hm_command_map.json` and suggests setting only
`tested_in_session=true`; `runtime_validated` and `verification_level` remain
...
```json
{
  "params": {
    ...
    "model_path": "C:/path/to/hyper-dyna-mcp/runs/listener_smoke.hm",
    "timeout": 60
  }
}
```
...
{
  "success": true,
  "path": "C:/path/to/hyper-dyna-mcp/runs/listener_smoke.hm",
  "size": 22717
}
```
