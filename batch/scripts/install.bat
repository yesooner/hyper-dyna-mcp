@echo off
REM Hyper-Dyna-MCP Batch Installation Script
REM 此脚本将配置文件从模板复制到实际位置
...
REM 复制本地路径配置
if not exist "path\local_paths.yaml" (
    copy "batch\templates\local_paths.yaml" "path\local_paths.yaml"
    echo Created path\local_paths.yaml
) else (
    echo path\local_paths.yaml already exists, skipping...
)

REM 复制 HyperMesh 路径配置
if not exist "path\hypermesh_paths.yaml" (
    copy "batch\templates\hypermesh_paths.yaml" "path\hypermesh_paths.yaml"
...
REM 复制 LS-DYNA 路径配置
if not exist "path\lsdyna_paths.yaml" (
    copy "batch\templates\lsdyna_paths.yaml" "path\lsdyna_paths.yaml"
    echo Created path\lsdyna_paths.yaml
) else (
    echo path\lsdyna_paths.yaml already exists, skipping...
)

REM 复制 LS-PrePost 路径配置
if not exist "path\lsprepost_paths.yaml" (
    copy "batch\templates\lsprepost_paths.yaml" "path\lsprepost_paths.yaml"
    echo Created path\lsprepost_paths.yaml
) else (
    echo path\lsprepost_paths.yaml already exists, skipping...
)

REM 复制 Obsidian 路径配置
if not exist "path\obsidian_paths.yaml" (
    copy "batch\templates\obsidian_paths.yaml" "path\obsidian_paths.yaml"
...
REM 复制 .env 文件
if not exist ".env" (
    copy "batch\templates\.env.example" ".env"
    echo Created .env
) else (
